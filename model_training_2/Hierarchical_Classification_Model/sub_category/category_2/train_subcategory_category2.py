...
def main():
    ...
    print("\n===== SUBCATEGORY MODEL (CATEGORY = 2) =====")

    df_train = load_table(DB_PATH, TABLE_TRAIN)
    df_test = load_table(DB_PATH, TABLE_TEST)

    # Filter to category=2
    df_train = df_train[df_train[CATEGORY_COL] == TARGET_CATEGORY]
    df_test = df_test[df_test[CATEGORY_COL] == TARGET_CATEGORY]

    # ----------------------------------------------
    # REMOVE UNWANTED SUBCATEGORY 28 FROM THIS MODEL
    # ----------------------------------------------
    UNUSED_LABELS = {28}
    df_train = df_train[~df_train[SUBCAT_COL].isin(UNUSED_LABELS)]
    df_test = df_test[~df_test[SUBCAT_COL].isin(UNUSED_LABELS)]
    # ----------------------------------------------
    ...
    joblib.dump(lr, os.path.join(MODEL_DIR, "lr_subcat_cat2.pkl"))
    ...
    joblib.dump(rf, os.path.join(MODEL_DIR, "rf_subcat_cat2.pkl"))
    ...
    xgb.save_model(os.path.join(MODEL_DIR, "xgb_subcat_cat2.json"))

    # ============================
    # EVALUATION
    # ============================

    report_path = os.path.join(MODEL_DIR, "report_subcat_cat2.txt")
    with open(report_path, "w", encoding="utf-8") as f:

        f.write("=== SUBCATEGORY MODEL (CATEGORY = 2) ===\n\n")
    ...
    print("==========================================")
    print("✔ SUBCATEGORY MODEL (CATEGORY=2) TRAINED")
