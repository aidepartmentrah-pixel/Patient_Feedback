...
def get_embedding(text: str, Troubleshoot = False):
    """
    Load MPNet model from local directory (inside function),
    but only once — all later calls reuse it.
    """
    global _TOKENIZER, _MODEL

    # -----------------------------------------
    #  Directories INSIDE the function
    # -----------------------------------------
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    MODEL_PATH = os.path.abspath(
        os.path.join(BASE_DIR, "..", "model_storage", "mpnet_embeddings")
    )
