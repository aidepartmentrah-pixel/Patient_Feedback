...
cursor.execute('SELECT TOP 1 CaseStageID FROM APP_LOOKUP_CASE_STAGE')
...
print("\n[STEP 2] Check APP database for record...")
try:
    conn = get_connection()
    cursor = conn.cursor()
    ...
    cursor.execute("""
        SELECT TOP 1
            CaseID, PatientName, BuildingID, ExplanationStatusID, FeedbackRecievedDate
    ...
    row = cursor.fetchone()
    if row:
        print(f"  Last record in APP DB:")
        print(f"    CaseID: {row[0]}")
