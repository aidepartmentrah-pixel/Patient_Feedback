# Patient Feedback System — Release 1.2.0

**Release date:** 2026-08-18
**Prepared by:** RAH Lab
**Application Release:** 1.2.0 (release engineering / deployment infrastructure — application behavior unchanged from 1.1.1)
...
|---|---|---|
| Backend | 1.2.0 | `rah-pfms-backend:1.2.0` (content identical to 1.1.1, retagged) |
| Frontend | 1.2.0 | `rah-pfms-frontend:1.2.0` (nginx config changed for HTTPS — see below) |
| DB init/migrate | 1.2.0 | content unchanged since 1.1.1, retagged to match this release's shared `APP_VERSION` |

## What's new

This release closes the RAH Application Release Engineering gap (Prompt 3
of the RAH Application Engineering Playbook) — it's an infrastructure/
deployment-process release, not an application-feature release. No page,
API behavior, or database schema changed from 1.1.1.

- **Canonical deployment path.** The live deployment now always lives at
  `/opt/rah/apps/pfms/`, completely independent of wherever this release
  folder is copied or later moved/removed. `install_offline.sh`/
  `update_offline.sh` were rewritten around a new shared `scripts/_common.sh`
  to enforce this consistently. Verified: install, update, and
  release-folder-independence (moved the release folder away entirely after
  install — the live deployment kept running).
- **Auto-generated credentials.** First install no longer requires the
  operator to hand-fill `MSSQL_SA_PASSWORD`, `SETTINGS_ENCRYPTION_KEY`, or
  `PASSWORD_EXPORT_ENCRYPTION_KEY` — see "Password handling" below for the
  SA password specifically. Only `MSSQL_PID` (SQL Server licensing edition)
  and `SERVER_HOSTNAME_OR_IP` (see HTTPS below) still require operator
  input — both are decisions only the operator can make.
- **Operational documentation vault.** Install/update now maintain
  `/opt/rah/documentation/Applications/pfms/current-state.md` (always
  reflects right now) and `deployment-history.md` (append-only log of
  every install/update).
- **DBeaver connection auto-provisioning.** If DBeaver is already present
  on the server, install/update write/merge a ready-to-use (passwordless)
  connection into its config automatically — never touches other saved
  connections, always backs up the config file first. Falls back cleanly
  to the existing manual guide (`DBEAVER_GUIDE.md`) if `jq` or DBeaver
  itself isn't present.
- **HTTPS.** The application is now served over HTTPS with a self-signed
  certificate, generated once per deployment and persisted across updates
  (same handling as `.env`/`assets`/`backups`). Plain HTTP now redirects to
  HTTPS. This is required, not cosmetic — the Speech-to-Text microphone
  feature needs a secure browser context to function at all. See
  `HTTPS_CLIENT_TRUST_GUIDE.md` for getting clinical workstation browsers
  to trust the certificate.
  - New `.env` fields: `SERVER_HOSTNAME_OR_IP` (operator-provided — the
    address workstations actually use to reach this server) and
    `FRONTEND_HTTPS_PORT` (defaults to **8102**, OR-LAB's previously-reserved
    "spare" port, now used for this).
- **Release manifest.** `release-manifest.json` at the release root —
  machine-readable image digests/checksums/sizes and the source git commit,
  generated by `scripts/generate_release_manifest.sh`. Deliberately doesn't
  duplicate this file's own "what's new" narrative.

## Upgrade note

**Requires one manual step before running `update_offline.sh`** on an
existing pre-1.2.0 deployment: add `SERVER_HOSTNAME_OR_IP=<this server's
real LAN IP or hostname>` to the live deployment's `.env`
(`/opt/rah/apps/pfms/.env`) first. `update_offline.sh` checks for this and
stops cleanly with instructions if it's missing, before touching anything —
it will not leave a deployment half-updated. Once present,
`update_offline.sh` handles the rest automatically, including generating a
certificate for the first time if this deployment has never had one (a
one-time event on the update that first introduces HTTPS; on every update
after that, the existing certificate is left untouched).

No database migration applies. `FRONTEND_HTTPS_PORT` does not need to be
manually added — it defaults to 8102 if absent.

---

# Patient Feedback System — Release 1.1.1

**Release date:** 2026-08-17
**Prepared by:** RAH Lab
**Application Release:** 1.1.1 (reporting + Hospital Directory API fixes — backward compatible with 1.1.0/1.0.0)
**Infrastructure Release compatibility:** RAH-OIP 1.0.0 or later
**Database schema:** unchanged — no migration required, safe to update in place

## Contents

| Component | Version | Notes |
|---|---|---|
| Backend | 1.1.1 | `rah-pfms-backend:1.1.1` |
| Frontend | 1.1.1 | `rah-pfms-frontend:1.1.1` |
| DB init/migrate | 1.1.1 | content unchanged since 1.0.0 — retagged to match this release's shared `APP_VERSION` |

## What's new

Application-code-only release. No database schema changes, no new environment
variables/secrets, no new infrastructure components — a straight image swap
over an existing 1.1.0 (or 1.0.0) install.

- **Reporting**: monthly report month names switched from Modern Standard
  Arabic to Levantine/Syriac Arabic (e.g. `آب` instead of `أغسطس` for
  August) — matches the regional Arabic actually used in Lebanon/Bilad
  al-Sham, not textbook MSA. New `levantineDate.js` frontend utility backs
  this consistently across the UI as well as generated report documents.
  Related fixes to the stylish Word report formatter and multi-report export
  service.
- **Hospital Directory API integration**:
  - The "Test Connection" flow in Settings > Config now correctly
    distinguishes "never attempted" (not configured / bad URL) from
    "attempted and failed" (`attempted: true/false` on `check_health()`'s
    result) — the Config page form was showing a misleading state before
    this.
  - HTTP 422 responses from the Hospital Directory API are now handled the
    same as 400 (bad request) instead of falling through to a generic,
    less useful "http_error" bucket.
- Supporting fixes to `EntitySearchBox`, `SearchableSelect`, and several
  report-filter/action-tracking components on the frontend; new admin
  concept/functionality user-guide documentation.

## Upgrade note

Drop-in image update over an existing 1.1.0 or 1.0.0 deployment — run
...
### Infrastructure credentials (`.env`, auto-generated by `install_offline.sh`)

On first run, `install_offline.sh` auto-fills `.env` so the operator never
hand-types or has to remember a secret:

- **`MSSQL_SA_PASSWORD`** is set to a **fixed** value (`NewPassword2004`),
  not randomly generated, by deliberate operator choice — every deployment
  of this release shares the same SQL Server admin password. This is a
  known tradeoff: it's simpler to operate (nothing to look up, no risk of
  losing a generated password), at the cost of that password being uniform
  across every site this release is installed at. If this release is ever
  installed at more than one site, or `.env` is ever exposed, treat this as
  a shared secret accordingly.
- **`SETTINGS_ENCRYPTION_KEY`** and **`PASSWORD_EXPORT_ENCRYPTION_KEY`** are
  each a freshly-generated, random Fernet key, unique per deployment —
  these are cryptographic keys (not human-facing passwords), so unlike the
  SA password there is no benefit to a fixed value and a real security cost
  to sharing one across deployments (a leak at one site would expose
  another's encrypted data). Generated with `openssl rand 32 | base64 -w0 |
  tr '+/' '-_'`, confirmed to produce a format Python's
  `cryptography.fernet.Fernet` accepts identically to its own
  `Fernet.generate_key()`.
- **`APP_VERSION`** is auto-filled from the `IMAGE_VERSION` file at the
  release root -- deliberately NOT the release folder's own name. The
  folder name (e.g. `release/1.2.0/`) can be ahead of what's actually baked
  into `docker-images/*.tar` mid-development; `IMAGE_VERSION` always
  reflects what was really built, which is what Compose needs to pull the
  right tag.
- **`MSSQL_PID`** (SQL Server licensing edition) is the only value
  `install_offline.sh` still stops and asks the operator to set — it's a
  licensing decision, not something safe to auto-pick.
