"""
train_harm_level_model.py

Training script for "harm_level" using embedding_text1.

Outputs in this folder:
- harm_level_logreg.pkl
- harm_level_rf.pkl
- harm_level_metrics.txt
- harm_level_confusion_matrix_logreg.png
- harm_level_confusion_matrix_rf.png

Usage:
    python train_harm_level_model.py
"""
...
# Output filenames
MODEL_LOGREG = HERE / "harm_level_logreg.pkl"
MODEL_RF = HERE / "harm_level_rf.pkl"
REPORT_PATH = HERE / "harm_level_metrics.txt"
CM_LOGREG = HERE / "harm_level_confusion_matrix_logreg.png"
CM_RF = HERE / "harm_level_confusion_matrix_rf.png"
...
def parse_embedding_series(series: pd.Series) -> np.ndarray:
    """Convert a pandas Series of JSON arrays (or Python lists) into a 2D numpy array."""
    out = []
    ...
    try:
        ...
        for name, df in (("train", df_train), ("test", df_test)):
            if EMBED_COL not in df.columns:
                raise KeyError(f"Embedding column '{EMBED_COL}' not in {name} table")
            if TARGET_COL not in df.columns:
                raise KeyError(f"Target column '{TARGET_COL}' not in {name} table")
        ...
        def clean_target(series, name):
            series = pd.to_numeric(series, errors="coerce")  # convert invalid strings to NaN
            n_missing = series.isna().sum()
            if n_missing > 0:
                print(f"⚠️ {n_missing} missing or invalid values found in {name}['{TARGET_COL}'], dropping those rows.")
                series = series.dropna()
            return series.astype(int)

        df_train = df_train.dropna(subset=[EMBED_COL])
        df_test = df_test.dropna(subset=[EMBED_COL])

        y_train = clean_target(df_train[TARGET_COL], "train")
        y_test = clean_target(df_test[TARGET_COL], "test")

        X_train = parse_embedding_series(df_train.loc[y_train.index, EMBED_COL])
        X_test = parse_embedding_series(df_test.loc[y_test.index, EMBED_COL])
        ...
        with open(REPORT_PATH, "w", encoding="utf-8") as f:
            ...
            for name, res in results.items():
                ...
                f.write(res["report"] + "\n\n")
            f.write("Note: harm_level labels are integer encoded.\n")
        ...
        save_confusion_matrix(cm_log, labels=label_names, out_path=CM_LOGREG,
