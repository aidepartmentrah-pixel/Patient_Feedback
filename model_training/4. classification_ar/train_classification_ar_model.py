"""
train_classification_ar_model.py

Training script for "classification_ar" using embedding_text1.

Outputs:
- classification_ar_logreg.pkl
- classification_ar_rf.pkl
- classification_ar_metrics.txt
- classification_ar_confusion_matrix_logreg.png
- classification_ar_confusion_matrix_rf.png

Usage:
    python train_classification_ar_model.py
"""
...
TARGET_COL = "classification_ar"

MODEL_LOGREG = HERE / "classification_ar_logreg.pkl"
MODEL_RF = HERE / "classification_ar_rf.pkl"
REPORT_PATH = HERE / "classification_ar_metrics.txt"
CM_LOGREG = HERE / "classification_ar_confusion_matrix_logreg.png"
CM_RF = HERE / "classification_ar_confusion_matrix_rf.png"
...
def parse_embedding_series(series: pd.Series) -> np.ndarray:
    """Convert stored embedding JSON/list strings into a 2D numpy array."""
    out = []
...
def main():
    try:
        ...
        # --- Clean labels to avoid NaN / string issues ---
        ...
        # Drop rows with missing target BEFORE extracting embeddings
        ...
        # Now extract embeddings (same rows only)
        X_train = parse_embedding_series(df_train[EMBED_COL])
        X_test = parse_embedding_series(df_test[EMBED_COL])

        # Clean targets
        y_train = clean_labels(df_train[TARGET_COL]).to_numpy()
        y_test = clean_labels(df_test[TARGET_COL]).to_numpy()

        # Safety check
        assert len(X_train) == len(y_train), f"Mismatch: {len(X_train)} != {len(y_train)}"
        assert len(X_test) == len(y_test), f"Mismatch: {len(X_test)} != {len(y_test)}"
        ...
        with open(REPORT_PATH, "w", encoding="utf-8") as f:
            f.write("Classification_AR results\n\n")
            for name, res in results.items():
                ...
                f.write(res["report"] + "\n\n")
            f.write("Note: classification_ar labels are integer encoded.\n")
        ...
        save_confusion_matrix(cm_log, label_names, CM_LOGREG, "Confusion Matrix - Logistic Regression")
        save_confusion_matrix(cm_rf, label_names, CM_RF, "Confusion Matrix - Random Forest")
