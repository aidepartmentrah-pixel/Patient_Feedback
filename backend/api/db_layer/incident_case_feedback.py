...
def create_subcase_rca_feedback(
    subcase_id: int,
    incident_id: int,
    feedback_data: dict,
    created_by_user_id: int,
    ...
    """
    Create RCA (Root Cause Analysis) feedback record linked to a subcase.
    
    This is used when section submits response from inbox.
    Each subcase can have one RCA record.
    
    Args:
        subcase_id: The administrative subcase ID
        incident_id: The parent incident case ID
        feedback_data: Dictionary with RCA fields (Cause_*, Preventive_*, etc.)
...
def get_rca_feedback_by_subcase(subcase_id: int) -> Optional[dict]:
    """
    Get RCA feedback record for a specific subcase.
    
    Args:
        subcase_id: The administrative subcase ID
        
    Returns:
        Dictionary with RCA fields or None if not found
    """
    conn = get_connection()
    cursor = conn.cursor()
