"""
ML Architecture Consolidation — Stage 8: Post-Freeze Baseline

Read-only against the live system. The Stage 1 baseline
(C:\\SQLBackup\\ml_stage1_archive_*) was taken before case_service.py's legacy
add_corrected_record_to_ml() call was removed, so it is not the right
"nothing changes after this point" reference for Stage 8's own migration
verification. This script takes a second, later checksummed archive copy of
both SQLite files plus a fresh SQL Server .bak, immediately after the freeze
landed — this is the snapshot Stage 8's migration verification compares
against.

Run from the backend/ directory:
    python -m scripts.ml_stage8_freeze_baseline
...
def backup_sql_server(timestamp: str) -> dict:
    bak_filename = f"IncidentManager_ml_stage8_freeze_{timestamp}.bak"
...
def main():
    ...
    archive_dir = os.path.join(ARCHIVE_ROOT, f"ml_stage8_freeze_baseline_{timestamp}")
    os.makedirs(archive_dir, exist_ok=True)

    print("=" * 70)
    print("ML ARCHITECTURE CONSOLIDATION — STAGE 8: POST-FREEZE BASELINE")
    print("=" * 70)
    ...
    try:
        sql_info = backup_sql_server(timestamp)
        ...
        print(f"  Fall back to a manual .bak via SSMS if this script lacks BACKUP DATABASE permission")
        print(f"  (this happened at Stage 1 too — see stage1_manifest.json).")
        manifest["sql_server"] = {"status": "failed", "error": str(e)}

    # --- Write manifest ---
    manifest_path = os.path.join(archive_dir, "stage8_freeze_baseline_manifest.json")
    with open(manifest_path, "w", encoding="utf-8") as f:
        json.dump(manifest, f, indent=2, default=str)

    print(f"\n{'=' * 70}")
    print(f"Manifest written to: {manifest_path}")
    print("This is the baseline Stage 8's own migration verification compares against.")
    print("=" * 70)
